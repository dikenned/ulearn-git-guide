...
- `git push -f` — выполнить push, even if local branch is not a descendant of remote branch
- `git push --force-with-lease` — выполнить push, even if local branch is not a descendant of remote branch, but on condition that remote branch hasn't moved (always use instead of previous command)
